...
import MovieCardXX from "../HomePage/FiltredCardsRender";

const MainMobile: FunctionComponent = () => {
  const dispatch = useDispatch();
  const searchTerm = useSelector((state:any) => state.searchTerm); 
  const movies = useSelector((state:any) => state.movies); // Добавлено
  ...
  const handleShowResults = () => {
    if (genre !== prevGenre) {
      toast("Фильтр жанра изменен");
      setPrevGenre(genre);
    }
    if (year !== prevYear) {
      toast("Фильтр года изменен");
      setPrevYear(year);
    }
    if (genre && year) {
      toast("Фильтры установлены");
          ...
          <section className="frameX">
            <div className="frameParent">
              ...
              <button className="rectangleGroup">
                <div className="frameItemRes"><div className="clearFilter">Clear filter</div></div>
              </button>
              </Link>
            </div>
          </section>
